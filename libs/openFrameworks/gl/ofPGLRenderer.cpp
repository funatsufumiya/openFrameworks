...
const std::string ofPGLRenderer::TYPE = "GL";

//----------------------------------------------------------
ofPGLRenderer::ofPGLRenderer(const ofAppBaseWindow * _window)
	: matrixStack(_window)
	, graphics3d(this) {
	std::cout << "Using ofPGLRenderer" << std::endl;
...
void ofPGLRenderer::startRender() {
	currentFramebufferId = defaultFramebufferId;
...
void ofPGLRenderer::finishRender() {
	matrixStack.clearStacks();
	framebufferIdStack.clear();
}

//----------------------------------------------------------
void ofPGLRenderer::draw(const ofMesh & vertexData, ofPolyRenderMode renderType, bool useColors, bool useTextures, bool useNormals) const {
	if (currentStyle.smoothing) const_cast<ofPGLRenderer *>(this)->startSmoothing();
...
#endif
	if (currentStyle.smoothing) const_cast<ofPGLRenderer *>(this)->endSmoothing();
}

//----------------------------------------------------------
void ofPGLRenderer::draw(const ofVboMesh & mesh, ofPolyRenderMode renderType) const {
	drawInstanced(mesh, renderType, 1);
}

//----------------------------------------------------------
void ofPGLRenderer::drawInstanced(const ofVboMesh & mesh, ofPolyRenderMode renderType, int primCount) const {
...
//----------------------------------------------------------
void ofPGLRenderer::draw(const of3dPrimitive & model, ofPolyRenderMode renderType) const {
	const_cast<ofPGLRenderer *>(this)->pushMatrix();
	const_cast<ofPGLRenderer *>(this)->multMatrix(model.getGlobalTransformMatrix());
	if (model.isUsingVbo()) {
		draw(static_cast<const ofVboMesh &>(model.getMesh()), renderType);
	} else {
		draw(model.getMesh(), renderType);
	}
	const_cast<ofPGLRenderer *>(this)->popMatrix();
}

//----------------------------------------------------------
void ofPGLRenderer::draw(const ofNode & node) const {
	const_cast<ofPGLRenderer *>(this)->pushMatrix();
	const_cast<ofPGLRenderer *>(this)->multMatrix(node.getGlobalTransformMatrix());
	node.customDraw(this);
	const_cast<ofPGLRenderer *>(this)->popMatrix();
}

//----------------------------------------------------------
void ofPGLRenderer::draw(const ofPolyline & poly) const {
...
//----------------------------------------------------------
void ofPGLRenderer::draw(const ofPath & shape) const {
	ofFloatColor prevColor;
	if (shape.getUseShapeColor()) {
		prevColor = currentStyle.color;
	}
	ofPGLRenderer * mut_this = const_cast<ofPGLRenderer *>(this);
	if (shape.isFilled()) {
...
//----------------------------------------------------------
void ofPGLRenderer::draw(const ofImage & image, float x, float y, float z, float w, float h, float sx, float sy, float sw, float sh) const {
	if (image.isUsingTexture()) {
		const ofTexture & tex = image.getTexture();
		if (tex.isAllocated()) {
			const_cast<ofPGLRenderer *>(this)->bind(tex, 0);
			draw(tex.getMeshForSubsection(x, y, z, w, h, sx, sy, sw, sh, isVFlipped(), currentStyle.rectMode), OF_MESH_FILL, false, true, false);
			const_cast<ofPGLRenderer *>(this)->unbind(tex, 0);
		} else {
			ofLogWarning("ofPGLRenderer") << "drawing an unallocated texture";
		}
	}
}

//----------------------------------------------------------
void ofPGLRenderer::draw(const ofFloatImage & image, float x, float y, float z, float w, float h, float sx, float sy, float sw, float sh) const {
	if (image.isUsingTexture()) {
		const ofTexture & tex = image.getTexture();
		if (tex.isAllocated()) {
			const_cast<ofPGLRenderer *>(this)->bind(tex, 0);
			...
			ofLogWarning("ofPGLRenderer") << "draw(): texture is not allocated";
		}
	}
}

//----------------------------------------------------------
void ofPGLRenderer::draw(const ofShortImage & image, float x, float y, float z, float w, float h, float sx, float sy, float sw, float sh) const {
	if (image.isUsingTexture()) {
		const ofTexture & tex = image.getTexture();
		if (tex.isAllocated()) {
			const_cast<ofPGLRenderer *>(this)->bind(tex, 0);
...
//----------------------------------------------------------
void ofPGLRenderer::draw(const ofTexture & tex, float x, float y, float z, float w, float h, float sx, float sy, float sw, float sh) const {
	if (tex.isAllocated()) {
		const_cast<ofPGLRenderer *>(this)->bind(tex, 0);
		draw(tex.getMeshForSubsection(x, y, z, w, h, sx, sy, sw, sh, isVFlipped(), currentStyle.rectMode), OF_MESH_FILL, false, true, false);
		const_cast<ofPGLRenderer *>(this)->unbind(tex, 0);
...
//----------------------------------------------------------
void ofPGLRenderer::draw(const ofBaseVideoDraws & video, float x, float y, float w, float h) const {
	if (video.isInitialized() && video.isUsingTexture()) {
		const_cast<ofPGLRenderer *>(this)->bind(video);
		...
		const_cast<ofPGLRenderer *>(this)->unbind(video);
	}
}

//----------------------------------------------------------
void ofPGLRenderer::draw(const ofVbo & vbo, GLuint drawMode, int first, int total) const {
...
//----------------------------------------------------------
void ofPGLRenderer::drawElements(const ofVbo & vbo, GLuint drawMode, int amt, int offsetelements) const {
	if (vbo.getUsingVerts()) {
		vbo.bind();
#ifdef TARGET_OPENGLES
...
//----------------------------------------------------------
void ofPGLRenderer::drawInstanced(const ofVbo & vbo, GLuint drawMode, int first, int total, int primCount) const {
	if (vbo.getUsingVerts()) {
		vbo.bind();
#ifdef TARGET_OPENGLES
...
//----------------------------------------------------------
void ofPGLRenderer::drawElementsInstanced(const ofVbo & vbo, GLuint drawMode, int amt, int primCount) const {
	if (vbo.getUsingVerts()) {
		vbo.bind();
#ifdef TARGET_OPENGLES
		// todo: activate instancing once OPENGL ES supports instancing, starting with version 3.0
...
//----------------------------------------------------------
ofPath & ofPGLRenderer::getPath() {
	return path;
}

//----------------------------------------------------------
void ofPGLRenderer::bind(const ofBaseVideoDraws & video) {
	if (video.isInitialized() && video.isUsingTexture()) {
...
//----------------------------------------------------------
void ofPGLRenderer::unbind(const ofBaseVideoDraws & video) {
	if (video.isInitialized() && video.isUsingTexture()) {
...
//----------------------------------------------------------
void ofPGLRenderer::bind(const ofShader & shader) {
	glUseProgram(shader.getProgram());
}

//----------------------------------------------------------
void ofPGLRenderer::unbind(const ofShader & shader) {
	glUseProgram(0);
}

//----------------------------------------------------------
void ofPGLRenderer::begin(const ofFbo & fbo, ofFboMode mode) {
	pushView();
	pushStyle();
...
//----------------------------------------------------------
void ofPGLRenderer::end(const ofFbo & fbo) {
	unbind(fbo);
	matrixStack.setRenderSurface(*window);
	popStyle();
	popView();
...
//----------------------------------------------------------
void ofPGLRenderer::bind(const ofFbo & fbo) {
...
#ifndef TARGET_OPENGLES
//----------------------------------------------------------
void ofPGLRenderer::bindForBlitting(const ofFbo & fboSrc, ofFbo & fboDst, int attachmentPoint) {
...
//----------------------------------------------------------
void ofPGLRenderer::unbind(const ofFbo & fbo) {
...
//----------------------------------------------------------
void ofPGLRenderer::bind(const ofBaseMaterial & material) {
...
//----------------------------------------------------------
void ofPGLRenderer::unbind(const ofBaseMaterial &) {
...
//----------------------------------------------------------
void ofPGLRenderer::bind(const ofShadow & shadow) {
	ofLogWarning("ofPGLRenderer::bind(const ofShadow & shadow) : shadows are only supported via programmable renderer");
};
//----------------------------------------------------------
void ofPGLRenderer::bind(const ofShadow & shadow, GLenum aCubeFace) {
	ofLogWarning("ofPGLRenderer::bind(const ofShadow & shadow) : shadows are only supported via programmable renderer");
};

// does nothing, only programmable renderer supported
//----------------------------------------------------------
void ofPGLRenderer::unbind(const ofShadow & shadow) {};
//----------------------------------------------------------
void ofPGLRenderer::unbind(const ofShadow & shadow, GLenum aCubeFace) {};

//----------------------------------------------------------
void ofPGLRenderer::bind(const ofTexture & texture, int location) {
...
//----------------------------------------------------------
void ofPGLRenderer::unbind(const ofTexture & texture, int location) {
...
//----------------------------------------------------------
void ofPGLRenderer::bind(const ofCamera & camera, const ofRectangle & _viewport) {
	pushView();
	viewport(_viewport);
...
//----------------------------------------------------------
void ofPGLRenderer::unbind(const ofCamera & camera) {
	popView();
}

//----------------------------------------------------------
void ofPGLRenderer::pushView() {
	getCurrentViewport();
...
//----------------------------------------------------------
void ofPGLRenderer::viewport(ofRectangle viewport_) {
	viewport(viewport_.x, viewport_.y, viewport_.width, viewport_.height, isVFlipped());
}

//----------------------------------------------------------
void ofPGLRenderer::viewport(float x, float y, float width, float height, bool vflip) {
...
//----------------------------------------------------------
ofRectangle ofPGLRenderer::getCurrentViewport() const {
	getNativeViewport();
	return matrixStack.getCurrentViewport();
}

//----------------------------------------------------------
ofRectangle ofPGLRenderer::getNativeViewport() const {
	GLint viewport[4]; // Where The Viewport Values Will Be Stored
	glGetIntegerv(GL_VIEWPORT, viewport);

	ofPGLRenderer * mutRenderer = const_cast<ofPGLRenderer *>(this);
...
//----------------------------------------------------------
int ofPGLRenderer::getViewportWidth() const {
	return getCurrentViewport().width;
}

//----------------------------------------------------------
int ofPGLRenderer::getViewportHeight() const {
	return getCurrentViewport().height;
}

//----------------------------------------------------------
void ofPGLRenderer::setCoordHandedness(ofHandednessType handedness) {
}

//----------------------------------------------------------
ofHandednessType ofPGLRenderer::getCoordHandedness() const {
	return matrixStack.getHandedness();
}

//----------------------------------------------------------
void ofPGLRenderer::setOrientation(ofOrientation orientation, bool vFlip) {
	matrixStack.setOrientation(orientation, vFlip);
}

//----------------------------------------------------------
bool ofPGLRenderer::isVFlipped() const {
	return matrixStack.isVFlipped();
}

//----------------------------------------------------------
bool ofPGLRenderer::texturesNeedVFlip() const {
	return matrixStack.customMatrixNeedsFlip();
}

//----------------------------------------------------------
void ofPGLRenderer::setupScreenPerspective(float width, float height, float fov, float nearDist, float farDist) {
	float viewW, viewH;
	if (width < 0 || height < 0) {
...
//----------------------------------------------------------
void ofPGLRenderer::setupScreenOrtho(float width, float height, float nearDist, float farDist) {
	float viewW, viewH;
	if (width < 0 || height < 0) {
		ofRectangle currentViewport = getCurrentViewport();
...
void ofPGLRenderer::setupGraphicDefaults() {
	glEnableClientState(GL_VERTEX_ARRAY);
	glDisableClientState(GL_NORMAL_ARRAY);
	glDisableClientState(GL_COLOR_ARRAY);
	glDisableClientState(GL_TEXTURE_COORD_ARRAY);
...
//----------------------------------------------------------
void ofPGLRenderer::setupScreen() {
	setupScreenPerspective(); // assume defaults
}

//----------------------------------------------------------
void ofPGLRenderer::setCircleResolution(int res) {
...
void ofPGLRenderer::setPolyMode(ofPolyWindingMode mode) {
...
//our openGL wrappers
//----------------------------------------------------------
void ofPGLRenderer::pushMatrix() {
	glPushMatrix();
}

//----------------------------------------------------------
void ofPGLRenderer::popMatrix() {
	glPopMatrix();
}

//----------------------------------------------------------
void ofPGLRenderer::translate(const glm::vec3 & p) {
	glTranslatef(p.x, p.y, p.z);
}

//----------------------------------------------------------
void ofPGLRenderer::translate(float x, float y, float z) {
	glTranslatef(x, y, z);
}

//----------------------------------------------------------
void ofPGLRenderer::scale(float xAmnt, float yAmnt, float zAmnt) {
	glScalef(xAmnt, yAmnt, zAmnt);
}

//----------------------------------------------------------
void ofPGLRenderer::rotateDeg(float degrees, float vecX, float vecY, float vecZ) {
	glRotatef(degrees, vecX, vecY, vecZ);
}

//----------------------------------------------------------
void ofPGLRenderer::rotateXDeg(float degrees) {
	glRotatef(degrees, 1, 0, 0);
}

//----------------------------------------------------------
void ofPGLRenderer::rotateYDeg(float degrees) {
	glRotatef(degrees, 0, 1, 0);
}

//----------------------------------------------------------
void ofPGLRenderer::rotateZDeg(float degrees) {
	glRotatef(degrees, 0, 0, 1);
}

//same as ofRotateZ
//----------------------------------------------------------
void ofPGLRenderer::rotateDeg(float degrees) {
	glRotatef(degrees, 0, 0, 1);
}

//----------------------------------------------------------
void ofPGLRenderer::rotateRad(float radians, float vecX, float vecY, float vecZ) {
	glRotatef(glm::degrees(radians), vecX, vecY, vecZ);
}

//----------------------------------------------------------
void ofPGLRenderer::rotateXRad(float radians) {
	glRotatef(glm::degrees(radians), 1, 0, 0);
}

//----------------------------------------------------------
void ofPGLRenderer::rotateYRad(float radians) {
	glRotatef(glm::degrees(radians), 0, 1, 0);
}

//----------------------------------------------------------
void ofPGLRenderer::rotateZRad(float radians) {
	glRotatef(glm::degrees(radians), 0, 0, 1);
}

//same as ofRotateZ
//----------------------------------------------------------
void ofPGLRenderer::rotateRad(float radians) {
	glRotatef(glm::degrees(radians), 0, 0, 1);
}

//----------------------------------------------------------
void ofPGLRenderer::matrixMode(ofMatrixMode mode) {
	glMatrixMode(GL_MODELVIEW + mode);
	matrixStack.matrixMode(mode);
}

//----------------------------------------------------------
void ofPGLRenderer::loadIdentityMatrix(void) {
	loadMatrix(glm::mat4(1.0));
}

//----------------------------------------------------------
void ofPGLRenderer::loadMatrix(const glm::mat4 & m) {
...
//----------------------------------------------------------
void ofPGLRenderer::loadMatrix(const float * m) {
 ...
 */
glm::mat4 ofPGLRenderer::getCurrentMatrix(ofMatrixMode matrixMode_) const {
...
//----------------------------------------------------------
glm::mat4 ofPGLRenderer::getCurrentOrientationMatrix() const {
	return matrixStack.getOrientationMatrix();
}

//----------------------------------------------------------
void ofPGLRenderer::multMatrix(const glm::mat4 & m) {
	if (matrixStack.getCurrentMatrixMode() == OF_MATRIX_PROJECTION) {
...
//----------------------------------------------------------
void ofPGLRenderer::multMatrix(const float * m) {
	multMatrix(glm::make_mat4(m));
}

//----------------------------------------------------------
void ofPGLRenderer::loadViewMatrix(const glm::mat4 & m) {
	int matrixMode;
...
//----------------------------------------------------------
void ofPGLRenderer::multViewMatrix(const glm::mat4 & m) {
	ofLogError() << "mutlViewMatrix not implemented on fixed GL renderer";
}

//----------------------------------------------------------
glm::mat4 ofPGLRenderer::getCurrentViewMatrix() const {
	return matrixStack.getViewMatrix();
}

//----------------------------------------------------------
glm::mat4 ofPGLRenderer::getCurrentNormalMatrix() const {
	return glm::transpose(glm::inverse(getCurrentMatrix(OF_MATRIX_MODELVIEW)));
}

//----------------------------------------------------------
void ofPGLRenderer::setColor(const ofFloatColor & color) {
	setColor(color.r, color.g, color.b, color.a);
}

//----------------------------------------------------------
void ofPGLRenderer::setColor(const ofFloatColor & color, float _a) {
	setColor(color.r, color.g, color.b, _a);
}

//----------------------------------------------------------
void ofPGLRenderer::setColor(float r, float g, float b) {
...
//----------------------------------------------------------
void ofPGLRenderer::setColor(float r, float g, float b, float a) {
...
//----------------------------------------------------------
void ofPGLRenderer::setColor(float gray) {
	setColor(gray, gray, gray);
}

//----------------------------------------------------------
void ofPGLRenderer::setHexColor(int hexColor) {
...
//----------------------------------------------------------
void ofPGLRenderer::clear() {
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
}

//----------------------------------------------------------
void ofPGLRenderer::clear(float r, float g, float b, float a) {
	glClearColor(r, g, b, a);
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
}

//----------------------------------------------------------
void ofPGLRenderer::clear(float brightness, float a) {
	clear(brightness, brightness, brightness, a);
}

//----------------------------------------------------------
void ofPGLRenderer::clearAlpha() {
	glColorMask(0, 0, 0, 1);
...
//----------------------------------------------------------
void ofPGLRenderer::setBackgroundAuto(bool bAuto) {
	bBackgroundAuto = bAuto;
}

//----------------------------------------------------------
bool ofPGLRenderer::getBackgroundAuto() {
	return bBackgroundAuto;
}

//----------------------------------------------------------
ofFloatColor ofPGLRenderer::getBackgroundColor() {
	return currentStyle.bgColor;
}

//----------------------------------------------------------
void ofPGLRenderer::setBackgroundColor(const ofFloatColor & color) {
...
//----------------------------------------------------------
void ofPGLRenderer::background(const ofFloatColor & c) {
	setBackgroundColor(c);
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
}

//----------------------------------------------------------
void ofPGLRenderer::background(float brightness) {
	background(ofFloatColor(brightness));
}

//----------------------------------------------------------
void ofPGLRenderer::background(int hexColor, int _a) {
	int r = (hexColor >> 16) & 0xff;
	int g = (hexColor >> 8) & 0xff;
	int b = (hexColor >> 0) & 0xff;
...
//----------------------------------------------------------
void ofPGLRenderer::background(float r, float g, float b, float a) {
	background(ofFloatColor(r, g, b, a));
}

//----------------------------------------------------------
void ofPGLRenderer::setFillMode(ofFillFlag fill) {
...
//----------------------------------------------------------
ofFillFlag ofPGLRenderer::getFillMode() {
	if (currentStyle.bFill) {
		return OF_FILLED;
	} else {
		return OF_OUTLINE;
...
//----------------------------------------------------------
void ofPGLRenderer::setRectMode(ofRectMode mode) {
	currentStyle.rectMode = mode;
}

//----------------------------------------------------------
ofRectMode ofPGLRenderer::getRectMode() {
	return currentStyle.rectMode;
}

//----------------------------------------------------------
void ofPGLRenderer::setLineWidth(float lineWidth) {
...
//----------------------------------------------------------
void ofPGLRenderer::setPointSize(float pointSize) {
	currentStyle.pointSize = pointSize;
	glPointSize(pointSize);
}

//----------------------------------------------------------
void ofPGLRenderer::setDepthTest(bool depthTest) {
	if (depthTest) {
...
//----------------------------------------------------------
void ofPGLRenderer::setLineSmoothing(bool smooth) {
	currentStyle.smoothing = smooth;
}

//----------------------------------------------------------
void ofPGLRenderer::startSmoothing() {
#ifndef TARGET_OPENGLES
...
//----------------------------------------------------------
void ofPGLRenderer::endSmoothing() {
#ifndef TARGET_OPENGLES
	glPopAttrib();
#endif
}
...
//----------------------------------------------------------
void ofPGLRenderer::setBlendMode(ofBlendMode blendMode) {
	switch (blendMode) {
...
#else
		ofLogWarning("ofPGLRenderer") << "OF_BLENDMODE_SUBTRACT not currently supported on OpenGL ES";
#endif
		glBlendFunc(GL_SRC_ALPHA, GL_ONE);
		break;
	}
...
#else
		ofLogWarning("ofPGLRenderer") << "OF_BLENDMODE_MAX maybe not currently supported on OpenGL ES";
...
#else
		ofLogWarning("ofPGLRenderer") << "OF_BLENDMODE_MIN maybe not currently supported on OpenGL ES";
		// https://stackoverflow.com/questions/15169923/creating-a-gl-max-blend-equation-equivalent-using-other-methods
#endif
		break;
	}
...
//----------------------------------------------------------
void ofPGLRenderer::setBitmapTextMode(ofDrawBitmapMode mode) {
	currentStyle.drawBitmapMode = mode;
}

//----------------------------------------------------------
ofStyle ofPGLRenderer::getStyle() const {
	return currentStyle;
}

//----------------------------------------------------------
void ofPGLRenderer::pushStyle() {
...
//----------------------------------------------------------
void ofPGLRenderer::setStyle(const ofStyle & style) {
	//color
...
//----------------------------------------------------------
void ofPGLRenderer::setCurveResolution(int resolution) {
	currentStyle.curveResolution = resolution;
	path.setCurveResolution(resolution);
}

//----------------------------------------------------------
void ofPGLRenderer::enablePointSprites() {
...
//----------------------------------------------------------
void ofPGLRenderer::disablePointSprites() {
...
//----------------------------------------------------------
void ofPGLRenderer::enableAntiAliasing() {
	glEnable(GL_MULTISAMPLE);
}

//----------------------------------------------------------
void ofPGLRenderer::disableAntiAliasing() {
	glDisable(GL_MULTISAMPLE);
}

//----------------------------------------------------------
void ofPGLRenderer::drawLine(float x1, float y1, float z1, float x2, float y2, float z2) const {
...
//----------------------------------------------------------
void ofPGLRenderer::drawRectangle(float x, float y, float z, float w, float h) const {
	...
	// use smoothness, if requested:
	if (currentStyle.smoothing && !currentStyle.bFill) const_cast<ofPGLRenderer *>(this)->startSmoothing();
	...
	// use smoothness, if requested:
	if (currentStyle.smoothing && !currentStyle.bFill) const_cast<ofPGLRenderer *>(this)->endSmoothing();
}

//----------------------------------------------------------
void ofPGLRenderer::drawTriangle(float x1, float y1, float z1, float x2, float y2, float z2, float x3, float y3, float z3) const {
...
//----------------------------------------------------------
void ofPGLRenderer::drawCircle(float x, float y, float z, float radius) const {
...
//----------------------------------------------------------
void ofPGLRenderer::drawEllipse(float x, float y, float z, float width, float height) const {
...
//----------------------------------------------------------
void ofPGLRenderer::drawString(std::string textString, float x, float y, float z) const {

	ofPGLRenderer * mutThis = const_cast<ofPGLRenderer *>(this);
	float sx = 0;
	float sy = 0;
...
//----------------------------------------------------------
void ofPGLRenderer::drawString(const ofTrueTypeFont & font, std::string text, float x, float y) const {
	ofPGLRenderer * mutThis = const_cast<ofPGLRenderer *>(this);
...
//----------------------------------------------------------
void ofPGLRenderer::enableTextureTarget(const ofTexture & tex, int textureLocation) {
	glActiveTexture(GL_TEXTURE0 + textureLocation);
	glClientActiveTexture(GL_TEXTURE0 + textureLocation);
...
//----------------------------------------------------------
void ofPGLRenderer::disableTextureTarget(int textureTarget, int textureLocation) {
	glActiveTexture(GL_TEXTURE0 + textureLocation);
...
//----------------------------------------------------------
void ofPGLRenderer::disableLighting() {
...
//----------------------------------------------------------
void ofPGLRenderer::enableSeparateSpecularLight() {
#ifndef TARGET_OPENGLES
...
//----------------------------------------------------------
void ofPGLRenderer::disableSeparateSpecularLight() {
#ifndef TARGET_OPENGLES
...
//----------------------------------------------------------
bool ofPGLRenderer::getLightingEnabled() {
	return glIsEnabled(GL_LIGHTING);
}

//----------------------------------------------------------
void ofPGLRenderer::setSmoothLighting(bool b) {
	if (b)
...
//----------------------------------------------------------
void ofPGLRenderer::setGlobalAmbientColor(const ofFloatColor & c) {
	GLfloat cc[] = { c.r, c.g, c.b, c.a };
	glLightModelfv(GL_LIGHT_MODEL_AMBIENT, cc);
}

//----------------------------------------------------------
void ofPGLRenderer::enableLight(int lightIndex) {
	enableLighting();
	glEnable(GL_LIGHT0 + lightIndex);
}

//----------------------------------------------------------
void ofPGLRenderer::disableLight(int lightIndex) {
	if (lightIndex != -1) {
...
//----------------------------------------------------------
void ofPGLRenderer::setLightSpotlightCutOff(int lightIndex, float spotCutOff) {
	glLightf(GL_LIGHT0 + lightIndex, GL_SPOT_CUTOFF, spotCutOff);
}

//----------------------------------------------------------
void ofPGLRenderer::setLightSpotConcentration(int lightIndex, float exponent) {
	glLightf(GL_LIGHT0 + lightIndex, GL_SPOT_EXPONENT, exponent);
}

//----------------------------------------------------------
void ofPGLRenderer::setLightAttenuation(int lightIndex, float constant, float linear, float quadratic) {
...
//----------------------------------------------------------
void ofPGLRenderer::setLightAmbientColor(int lightIndex, const ofFloatColor & c) {
	if (lightIndex == -1) return;
	glLightfv(GL_LIGHT0 + lightIndex, GL_AMBIENT, &c.r);
}

//----------------------------------------------------------
void ofPGLRenderer::setLightDiffuseColor(int lightIndex, const ofFloatColor & c) {
	if (lightIndex == -1) return;
	glLightfv(GL_LIGHT0 + lightIndex, GL_DIFFUSE, &c.r);
}

//----------------------------------------------------------
void ofPGLRenderer::setLightSpecularColor(int lightIndex, const ofFloatColor & c) {
	if (lightIndex == -1) return;
	glLightfv(GL_LIGHT0 + lightIndex, GL_SPECULAR, &c.r);
}

//----------------------------------------------------------
void ofPGLRenderer::setLightPosition(int lightIndex, const glm::vec4 & position) {
	if (lightIndex == -1) return;
	int matrixMode;
	glGetIntegerv(GL_MATRIX_MODE, &matrixMode);
	glMatrixMode(GL_MODELVIEW);
...
//----------------------------------------------------------
void ofPGLRenderer::setLightSpotDirection(int lightIndex, const glm::vec4 & direction) {
	if (lightIndex == -1) return;
	int matrixMode;
	glGetIntegerv(GL_MATRIX_MODE, &matrixMode);
	glMatrixMode(GL_MODELVIEW);
...
//----------------------------------------------------------
int ofPGLRenderer::getGLVersionMajor() {
#ifdef TARGET_OPENGLES
	return 1;
#else
	return 2;
...
//----------------------------------------------------------
int ofPGLRenderer::getGLVersionMinor() {
#ifdef TARGET_OPENGLES
	return 0;
#else
	return 1;
...
//----------------------------------------------------------
void ofPGLRenderer::saveFullViewport(ofPixels & pixels) {
	ofRectangle v = getCurrentViewport();
	saveScreen(v.x, v.y, v.width, v.height, pixels);
}

//----------------------------------------------------------
void ofPGLRenderer::saveScreen(int x, int y, int w, int h, ofPixels & pixels) {
...
//----------------------------------------------------------
const of3dGraphics & ofPGLRenderer::get3dGraphics() const {
	return graphics3d;
}

//----------------------------------------------------------
of3dGraphics & ofPGLRenderer::get3dGraphics() {
	return graphics3d;
}
